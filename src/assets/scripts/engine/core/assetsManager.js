...
class AssetsManager {
    ...
    getAsset(assetType, assetName) {
        if (!this.assets[assetName] || !this.assets[assetName].isLoaded){
            Log.push('error', this, `Asset ${assetName} has not been loaded ! Make sure it is in assets.js with the correct name.`);
            return;
        } 
        switch (assetType) {
            case 'model':
